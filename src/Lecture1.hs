...
subString start end str = take (end - max 0 start + 1) (drop start str)
...
lowerAndGreater number list = show number ++ go 0 0 list
 where
    go :: Int -> Int -> [Int] -> String
    go greater smaller l
        | null l =  " is greater than " ++ show greater ++ " elements and lower than " ++ show smaller ++ " elements"
        | head l < number = go (greater + 1) smaller (tail l)
        | head l > number = go greater (smaller + 1) (tail l)
        | otherwise       = go greater smaller (tail l)
